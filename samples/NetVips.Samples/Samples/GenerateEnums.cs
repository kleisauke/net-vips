    ...
    private string Generate()
    {
        ...
        foreach (var name in allFlags)
        {
            ...
            stringBuilder.AppendLine("    /// <summary>")
                ...
                .AppendLine("    /// </summary>")
                .AppendLine("    [Flags]")
                .AppendLine($"    public enum {csharpName} : uint")
                .AppendLine("    {");

            var flagsValues = NetVips.ValuesForFlags(gtype);

            for (var i = 0; i < flagsValues.Count; i++)
            {
                var kvp = flagsValues.ElementAt(i);
                var flagsKey = kvp.Key.Replace('-', '_').ToPascalCase();

                stringBuilder.AppendLine($"        /// <summary>{flagsKey}</summary>")
                    .Append($"        {flagsKey} = {kvp.Value}")
                    .AppendLine((i != flagsValues.Count - 1 ? "," : string.Empty) + $" // \"{kvp.Key}\"");
            }
